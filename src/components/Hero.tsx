...
export default function Hero() {
  ...
  return (
    <section ref={ref} id="hero" onMouseMove={onMove} onMouseLeave={onLeave} className="relative min-h-screen flex items-center overflow-hidden">
      {/* BG layers */}
      ...
      <div className="relative z-10 container px-4 sm:px-6 pt-28 pb-20">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div>
            ...
            <BlurFade delay={0.2}>
              <h1 className="text-[2.5rem] sm:text-5xl md:text-6xl lg:text-7xl font-black leading-[1.02] tracking-tight mb-2">
                <span className="text-silver-gradient">Prime Rim</span><br />
                <span className="text-bronze-gradient">Repair</span>
                <span className="text-silver-gradient text-[0.55em]"> Mobile Service</span>
              </h1>
            </BlurFade>
            ...
            <BlurFade delay={0.4}>
              <div className="flex flex-wrap gap-4 mb-10">
                <a href="tel:7479557555" className="btn-bronze text-[14px] sm:text-[15px] uppercase tracking-[0.1em] px-8 sm:px-10 py-4 sm:py-5 pulse-cta">
                ...
                <a href="#services" className="btn-outline text-[12px] uppercase tracking-[0.1em] px-7 py-3.5">
                  Our Services
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M5 12h14M12 5l7 7-7 7" /></svg>
                </a>
                <a href="tel:7479557555" className="text-bronze-300 text-lg sm:text-xl font-bold tracking-wide hover:text-bronze-200 transition-colors">
                  (747) 955-7555
                </a>
              </div>
            </BlurFade>
            ...
            <BlurFade delay={0.5}>
              <div className="flex flex-wrap gap-4 sm:gap-8">
                {[
                  { val: "🚐 Mobile", lbl: "We Come To You" },
                  { val: "Same Day", lbl: "Service" },
                  { val: "5+", lbl: "Years Exp." },
                ].map((s) => (
                  <div key={s.lbl} className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-lg bg-bronze-500/10 border border-bronze-500/15 flex items-center justify-center">
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#c49332" strokeWidth="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" /></svg>
                    </div>
                    <div>
                      <p className="text-white font-bold text-sm leading-tight">{s.val}</p>
                      <p className="text-silver-500 text-[10px] tracking-wider uppercase">{s.lbl}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <BlurFade delay={0.3} className="flex justify-center mt-8 lg:mt-0">
            <div
              style={{
                transform: `perspective(1000px) rotateX(${tilt.rx}deg) rotateY(${tilt.ry}deg)`,
                transition: tilt.rx === 0 ? "transform 0.6s ease-out" : "transform 0.08s ease-out",
              ...
              <div className="relative w-[280px] h-[280px] sm:w-[360px] sm:h-[360px] lg:w-[480px] lg:h-[480px]">
                ...
                <motion.img
                  ...
                  style={{
                    ...
                    filter: "drop-shadow(0 30px 60px rgba(0,0,0,0.7)) drop-shadow(0 0 50px rgba(196,147,50,0.1)) sepia(0.25) saturate(1.3) brightness(0.75) hue-rotate(-15deg)",
                  }}
                />

                {/* Sparkle effects */}
                <motion.div animate={{ opacity: [0.15, 0.7, 0.15], scale: [0.8, 1.3, 0.8] }} transition={{ duration: 2.5, repeat: Infinity }} className="absolute top-[20%] right-[12%] w-3 h-3 bg-bronze-300 rounded-full blur-[3px] z-20" />
                <motion.div animate={{ opacity: [0.2, 0.6, 0.2] }} transition={{ duration: 3, repeat: Infinity, delay: 1 }} className="absolute top-[35%] right-[3%] w-2 h-2 bg-white/40 rounded-full blur-[2px] z-20" />
                <motion.div animate={{ opacity: [0.1, 0.5, 0.1] }} transition={{ duration: 2, repeat: Infinity, delay: 1.5 }} className="absolute bottom-[30%] left-[8%] w-2 h-2 bg-bronze-200/40 rounded-full blur-[2px] z-20" />
              </div>
            </div>
          </BlurFade>
        </div>
